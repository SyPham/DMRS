...
export class BpfcDetailComponent implements OnInit, AfterViewInit {
    ...
    if (args.requestType === 'save') {
      ...
      if (percentageChanged) {
        ...
        if (this.gridglue.getSelectedRecords().length > 0) {
          const glueIngredients = (this.gridglue.getSelectedRecords()[0] as any)
            .glueIngredients;
          if (glueIngredients.length > 0) {
            const index = glueIngredients.findIndex(
              (obj) => obj.ingredientID === args.data.id
            );
  ...
  LoadHistoryBPFC() {
    this.bPFCEstablishService.LoadHistoryBPFC(this.BPFCID)
    .subscribe((res: any) => {
  ...
  getArtProcessByArticleNoID(articleNoID) {
    this.artProcessService.getArtProcessByArticleNoID(articleNoID)
      .subscribe((res: any) => {
